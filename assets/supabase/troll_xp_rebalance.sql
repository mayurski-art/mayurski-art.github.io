-- ============================================================================
-- XP REBALANCE — retunes six existing troll_award_xp events. Run ONCE in
-- Supabase -> SQL Editor, AFTER troll_map_pin_xp.sql (idempotent).
--
-- Changes:
--   daily_login       10 XP  -> 20 XP   (cooldown/cap unchanged: 20h, 1/day)
--   chat_post          2 XP  ->  3 XP   (cooldown/cap unchanged: 2min, 20/day)
--   feedback_post       5 XP -> 100 XP  (cap unchanged at 2/day — still the
--                       ceiling on how much XP feedback can earn per day)
--   map_pin            15 XP -> 50 XP   (still once-ever, v_lifetime_cap = 1)
--   profile_avatar      25 XP -> 50 XP  (still once-ever, v_lifetime_cap = 1)
--   terminal_session   10 XP every ~20h -> 5 XP every message, no cooldown,
--                       no daily cap (was 1/day) — the terminal's chat route
--                       already calls troll_award_xp_service on every
--                       successful reply, so this just lets that award land
--                       every time instead of once a day.
--
-- Carries forward the whole allowlist from troll_map_pin_xp.sql (public
-- troll_award_xp) and troll_terminal_xp.sql (troll_award_xp_internal, the
-- service-role path the terminal actually calls) — every other event is
-- unchanged.
--
-- Also RESTORES the "Member" profile tag grant from troll_member_tag.sql,
-- which troll_map_pin_xp.sql accidentally dropped when it last replaced
-- troll_award_xp. Now applied to BOTH entry points (troll_award_xp and
-- troll_award_xp_internal), so a first-ever XP award grants the tag
-- whether it came from a browser or from the terminal's service-role path
-- — the original only covered the browser path. Requires troll_custom_tags.sql
-- to already exist (silently no-ops if the 'member' tag def hasn't been
-- created yet in admin.html -> Player accounts -> Profile tags).
-- ============================================================================

-- ---- Public entry point (browser clients, auth.uid()-based) --------------
...
begin
  ...
  case p_event
    ...
    when 'map_pin'            then v_xp := 50;  v_cooldown := interval '1 second'; v_daily_cap := 1; v_lifetime_cap := 1;
...
-- ---- Service-role path (troll_award_xp_service -> this) ------------------
-- This is what the terminal's chat route actually calls for
-- terminal_session, so its allowlist entry is the one that matters for
-- change #6.
