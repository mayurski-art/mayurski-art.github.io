-- ============================================================================
-- TROLL OPS XP — one shared level. Match XP earned in Troll Ops is credited
-- 1:1 to the account, and the game reads its rank (weapon unlocks included)
-- straight off troll_profiles.level. Run ONCE in Supabase -> SQL Editor,
-- AFTER troll_xp_rebalance.sql. Idempotent.
--
-- New event on the public troll_award_xp allowlist:
--   troll_ops_xp — variable. XP = p_meta.xp, the amount the game banked
--                  (per-kill PvP XP + match bonuses, or the Ops wave payout).
--                  No cooldown, no daily cap: the owner chose "no cap"
--                  knowing the amount is client-reported (Troll Ops is
--                  client-authoritative by design). The only check is that
--                  it's a positive integer.
--
-- Replaces troll_award_xp's whole body, so the allowlist below is carried
-- forward unchanged from troll_xp_rebalance.sql plus the one new event. If a
-- later migration replaces troll_award_xp again, it must keep troll_ops_xp.
-- ============================================================================
...
begin
  ...
  case p_event
    ...
    -- Variable: the amount the game banked. There is no server-side source to
    -- recompute it from, so the only check is a positive integer.
    when 'troll_ops_xp' then
      v_ops_xp := floor((p_meta->>'xp')::numeric)::integer;
      if v_ops_xp is null or v_ops_xp <= 0 then
        raise exception 'troll_ops_xp requires a positive xp in metadata';
      end if;
      v_xp := v_ops_xp;
      v_cooldown := interval '0 seconds';
      v_daily_cap := 100000000;
